...
impl MaterialMatcher {
    ...
    pub fn match_material(target_front_rgb: [f64; 3], target_edge_rgb: [f64; 3]) -> PbrMaterial {
        ...
        let genes = vec![
            Gene { name: "Albedo_R".to_string(), bounds: (0.0, 1.0), current_value: 0.5 },
            Gene { name: "Albedo_G".to_string(), bounds: (0.0, 1.0), current_value: 0.5 },
            Gene { name: "Albedo_B".to_string(), bounds: (0.0, 1.0), current_value: 0.5 },
            Gene { name: "Roughness".to_string(), bounds: (0.0, 1.0), current_value: 0.5 },
            Gene { name: "Metallic".to_string(), bounds: (0.0, 1.0), current_value: 0.0 },
        ];
        ...
        let (_, best_genes) = TheCrucible::anneal(
            ...
            |current_genes| {
                let mat = PbrMaterial {
                    albedo: [current_genes[0].current_value, current_genes[1].current_value, current_genes[2].current_value],
        ...
        let best_mat = PbrMaterial {
            albedo: [best_genes[0].current_value, best_genes[1].current_value, best_genes[2].current_value],
            roughness: best_genes[3].current_value,
            metallic: best_genes[4].current_value,
        };
        
        println!("✅ Material Match Complete!");
        println!("  -> Albedo: [{:.2}, {:.2}, {:.2}]", best_mat.albedo[0], best_mat.albedo[1], best_mat.albedo[2]);
