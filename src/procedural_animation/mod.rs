...
impl AnimationOptimizer {
    pub fn optimize_transition() -> AnimationCurve {
        ...
        for i in 1..=5 {
            genes.push(Gene { name: format!("K{}", i), bounds: (0.0, 1.0), current_value: (i as f64) / 6.0 });
        }
        ...
        let (_, best_genes) = TheCrucible::anneal(
            ...
            |g| {
                ...
                for i in 0..5 { vals.push(g[i].current_value); }
                vals.push(1.0);
        ...
        let mut kfs = vec![0.0];
        for g in best_genes { kfs.push(g.current_value); }
        kfs.push(1.0);
