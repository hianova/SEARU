pub mod attention {
    ...
    use no_std_tool::numerics::math::{FIXED_POINT_SHIFT, exp_approx_q16};
    #[doc = " CPU-bound Tiled FlashAttention (Zero-Float Base)"]
    #[doc = " Computes attention purely with integers, completely bypassing the FPU."]
    #[repr(C, align(64))]
    pub struct IntegerTiledAttention;
    impl IntegerTiledAttention {
        #[doc = " Executes a CPU-optimized Tiled Attention loop purely in i8/i32."]
        #[doc = " Removes libm, floating point softmax, and floating point scaling."]
        #[doc = " Parallelized over Query tiles with Rayon, and SIMD optimized via NEON."]
        pub fn compute_attention_i8(
            q: &[i8],
            k: &[i8],
            v: &[i8],
            seq_len: usize,
            ...
            output
                .chunks_mut(tile_size * head_dim)
                .enumerate()
                .for_each(|(t_q, out_tile)| {
                    ...
                    let mut s_ij = alloc :: vec ! [0i32 ; q_len * tile_size];
                    ...
                    for t_k in 0..num_tiles_k {
                            ...
                            for j in 0..k_len {
                                ...
                                unsafe {
                                    ...
                                    for c in 0..chunks {
                                        ...
                                        sum_vec =
                                            vaddq_s32(sum_vec, vaddq_s32(sum32_low, sum32_high));
                                    }
                        ...
                        for i in 0..q_len {
                            ...
                            let mut l_i_new =
                                ((l[i] as i64 * exp_diff as i64) >> FIXED_POINT_SHIFT) as i32;
                            for j in 0..k_len {
                            ...
                            for d in 0..head_dim {
                                let scaled_out = ((out_row[d] as i64 * exp_diff as i64)
                                    >> FIXED_POINT_SHIFT)
                                    as i32;
                                let mut pv = 0i64;
                                for j in 0..k_len {
                                    if p_ij[j] > 0 {
                                        pv +=
                                            p_ij[j] as i64 * v[(k_start + j) * head_dim + d] as i64;
                                    }
                                }
...
pub mod tokenizer {
    ...
    impl TrieTokenizer {
        ...
        #[doc = " Add a string/byte-sequence to the tokenizer vocabulary."]
        ...
        #[doc = " Encodes a string into token IDs using greedy longest-prefix matching."]
        ...
        #[doc = " Decodes a sequence of token IDs back into a string."]
        #[doc = " Invalid UTF-8 bytes are replaced with the replacement character."]
    ...
    mod tests {
        ...
        fn test_trie_tokenizer() {
            ...
            assert_eq!(encoded_unk, vec![0, 0, 0]);
        }
    }
}
#[cfg(test)]
