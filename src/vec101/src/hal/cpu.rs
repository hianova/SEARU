    ...
    fn compute(&self, ctx: &vec101_context) {
        ...
        if !is_gemm && num_threads <= 1 {
            ...
            let x_mask_slice = if use_scratch {
                unsafe {
                    let m = core::slice::from_raw_parts_mut(
                        ctx.scratch_buffer as *mut u64,
                        in_features / 64,
                    );
                    m.fill(0);
                    m
            ...
            for row in 0..ctx.num_rows {
                crate::compute::process_row_gemv_safe(row, ctx, x_mask_ref);
            }
            return;
        }
        let in_features = ctx.blocks_per_row * 2048;
        ...
        let x_mask_len = in_features / 64;
        let x_t_len = if is_gemm {
            in_features * padded_batch
        } else {
            0
        };
        let row_sums_len = if is_gemm {
            padded_batch * num_threads
        } else {
            0
        };
        ...
        let (x_mask_slice, x_t_slice, row_sums_slice) = if use_scratch {
            ...
            (
                fallback_x_mask.as_mut_slice(),
                fallback_x_t.as_mut_slice(),
                fallback_row_sums.as_mut_slice(),
            )
        };
        ...
        if use_threads {
            #[cfg(feature = "std")]
            {
                ...
                std::thread::scope(|s| {
                    for thread_idx in 0..(num_threads - 1) {
                        ...
                        std::thread::Builder::new()
                            .spawn_scoped(s, move || {
                                ...
                                let thread_row_sums_ptr =
                                    unsafe { sync_ptr.0.add(thread_idx * padded_batch) };
                                let thread_row_sums = unsafe {
                                ...
                                loop {
                                    ...
                                    if is_gemm {
                                        crate::compute::process_row_gemm_safe(
                                            row,
                                            thread_ctx,
                                            x_t_ref,
                                            x_mask_ref,
                                        ...
                                        crate::compute::process_row_gemv_safe(
                                            row, thread_ctx, x_mask_ref,
                                        );
                                    }
                                }
                            })
                    ...
                    let main_row_sums_ptr =
                        unsafe { row_sums_sync_ptr.0.add(main_thread_idx * padded_batch) };
                    let main_row_sums =
                        unsafe { core::slice::from_raw_parts_mut(main_row_sums_ptr, padded_batch) };
            ...
            let main_row_sums =
                unsafe { core::slice::from_raw_parts_mut(row_sums_sync_ptr.0, padded_batch) };
            for row in 0..ctx.num_rows {
                if is_gemm {
                    crate::compute::process_row_gemm_safe(
                        row,
