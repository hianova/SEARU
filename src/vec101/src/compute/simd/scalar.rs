...
#[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
pub unsafe fn process_row_scalar_gemv(
    row: usize,
    ...
    match context.quant_type {
        crate::compute::types::QuantType::Bit1_58 => unsafe {
            process_row_scalar_gemv_bit1_58(row, context, x_mask)
        },
    }
}
#[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
unsafe fn process_row_scalar_gemv_bit1_58(
    row: usize,
    context: &Vector101__Computation__Context,
    x_mask: &[u64],
) {
    ...
    if context.enable_liquid {
        ...
        let quantized =
            crate::compute::liquid_step_i8(out_val, context.dt, unsafe { &mut *state_ptr }, tau);
        unsafe {
...
unsafe fn process_row_scalar_gemm_bit1_58(
    ...
    for b in 0..context.batch_size {
        ...
        if context.enable_liquid {
            ...
            let quantized = crate::compute::liquid_step_i8(
                out_val,
                context.dt,
            ...
            unsafe {
                *context.liquid_out_buffer.add(b * context.num_rows + row) = quantized;
            }
        } else {
            unsafe {
                *context.out_buffer.add(b * context.num_rows + row) += out_val;
            }
        }
    }
}
