...
use crate::compute::types::Vector101__Computation__Context;
...
#[cfg(target_arch = "aarch64")]
#[doc = " # Safety"]
#[doc = " The caller must ensure that `row` is within bounds and `context` pointers are valid."]
pub unsafe fn process_row_neon_gemv(
    row: usize,
    ...
    match context.quant_type {
        crate::compute::types::QuantType::Bit1_58 => unsafe {
            process_row_neon_gemv_bit1_58(row, context, x_mask)
        },
    }
}
#[cfg(target_arch = "aarch64")]
unsafe fn process_row_neon_gemv_bit1_58(
    row: usize,
    context: &Vector101__Computation__Context,
    _x_mask: &[u64],
) {
    ...
    for col in 0..context.blocks_per_row {
        ...
        let w_super = unsafe {
            &(*(context.w_stream as *const crate::compute::types::Vec101SuperBlock).add(block_idx))
        };
        for sub_blk in 0..8 {
            ...
            for sub in 0..8 {
                ...
                let mask_pos_lo =
                    unsafe { expand_bits_to_mask_neon((w_pos_32 & 65535) as u16, bit_mask) };
                let mask_pos_hi =
                    unsafe { expand_bits_to_mask_neon((w_pos_32 >> 16) as u16, bit_mask) };
                let mask_neg_lo =
                    unsafe { expand_bits_to_mask_neon((w_neg_32 & 65535) as u16, bit_mask) };
                let mask_neg_hi =
                    unsafe { expand_bits_to_mask_neon((w_neg_32 >> 16) as u16, bit_mask) };
    ...
    if context.enable_liquid {
        ...
        let quantized =
            crate::compute::liquid_step_i8(out_val, context.dt, &mut unsafe { *state_ptr }, tau);
        unsafe {
            *context.liquid_out_buffer.add(row) = quantized;
        }
    } else {
        let out_ptr = unsafe { context.out_buffer.add(row) };
        unsafe {
            *out_ptr = (*out_ptr).saturating_add(out_val);
        }
    }
}
#[cold]
...
#[doc = " # Safety"]
#[doc = " The caller must ensure that `row` is within bounds and `context` pointers are valid."]
pub unsafe fn process_row_neon_gemm(
    row: usize,
    context: &Vector101__Computation__Context,
    _x_t: &[i8],
    _x_mask: &[u64],
    ...
    match context.quant_type {
        crate::compute::types::QuantType::Bit1_58 => unsafe {
            process_row_neon_gemm_bit1_58(row, context, _x_t, _x_mask, _padded_batch, row_sums)
        },
    }
}
#[cfg(target_arch = "aarch64")]
unsafe fn process_row_neon_gemm_bit1_58(
    row: usize,
    context: &Vector101__Computation__Context,
    _x_t: &[i8],
    _x_mask: &[u64],
    ...
    let scale = unsafe { *context.s_stream.add(row) };
    let in_features = context.blocks_per_row * 2048;
    let bit_mask_arr: [u8; 16] = [1, 2, 4, 8, 16, 32, 64, 128, 1, 2, 4, 8, 16, 32, 64, 128];
    let bit_mask = unsafe { vld1q_u8(bit_mask_arr.as_ptr()) };
    row_sums[..context.batch_size].fill(0);
    for col in 0..context.blocks_per_row {
        let block_idx = row * context.blocks_per_row + col;
        let w_super = unsafe {
            &(*(context.w_stream as *const crate::compute::types::Vec101SuperBlock).add(block_idx))
        ...
        for sub_blk in 0..8 {
            ...
            let mut b_idx = 0;
            while b_idx + 3 < context.batch_size {
                let ptr0 = unsafe { context.x_stream.add(b_idx * in_features) };
                let ptr1 = unsafe { context.x_stream.add((b_idx + 1) * in_features) };
                let ptr2 = unsafe { context.x_stream.add((b_idx + 2) * in_features) };
                let ptr3 = unsafe { context.x_stream.add((b_idx + 3) * in_features) };
            ...
            while b_idx < context.batch_size {
                let x_batch_ptr = unsafe { context.x_stream.add(b_idx * in_features) };
                let mut acc = unsafe { vdupq_n_s32(0) };
                for chunk in 0..16 {
                    let offset = col * 2048 + sub_blk * 256 + chunk * 16;
    ...
    for (b, &sum) in row_sums.iter().enumerate().take(context.batch_size) {
        unsafe {
            *context.out_buffer.add(b * context.num_rows + row) +=
                ((sum as i64 * scale as i64) >> 16) as i32;
        }
    }
}
