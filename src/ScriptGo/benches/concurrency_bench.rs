...
fn bench_hot_reload_contention(c: &mut Criterion) {
    ...
    let writer_handle = thread::Builder::new()
        ...
        .spawn(move || {
            no_std_tool::diagnostics::debug::track_thread_spawn();
            ...
            while writer_running.load(Ordering::Relaxed) {
                ...
                toggle = !toggle;
                thread::sleep(std::time::Duration::from_micros(covopt_param!(
                    "M_91_63", 10
                )));
            }
            no_std_tool::diagnostics::debug::track_thread_exit();
        })
        .unwrap();
    ...
    assert!(
        no_std_tool::diagnostics::debug::check_thread_drops(),
