...
use no_std_tool::structures::compress::{
    TimeSeriesEncoder, TimestampEncoder, leb128_encode_u32, zigzag_encode_i32,
};
...
impl EdgeNetworkFirewall {
    ...
    pub fn evaluate_packet(&mut self, packet_header: &NetworkPacketHeader) -> PacketFilterDecision {
        ...
        let tracker_index = ((source_ip ^ (source_ip >> covopt_param!("M_89_56", 16))) as usize)
            % covopt_param!("M_89_73", 256);
...
impl ZeroAllocationFlowLogEncoder {
    ...
    pub fn encode_log_entry(
        ...
        let packed_action_protocol = ((filter_decision.rule_action as u32)
            << covopt_param!("M_171_78", 8))
...
fn generate_benchmark_packet_batch(packet_count: usize) -> Vec<NetworkPacketHeader> {
    ...
    for index in 0..packet_count {
        let source_ip_address = match index % covopt_param!("M_192_46", 10) {
            ...
            _ => {
                covopt_param!("M_196_17", 3232235778)
                    + (index % covopt_param!("M_196_39", 250)) as u32
            }
        };
        ...
        let protocol_number = if index % covopt_param!("M_211_41", 100) == 0 {
            covopt_param!("M_211_52", 255)
        } else {
            covopt_param!("M_211_65", 6)
        };
        let packet_timestamp = base_timestamp
            + (index as u64 * covopt_param!("M_212_64", 100))
            + (index % covopt_param!("M_212_80", 5)) as u64;

        packet_headers.push(NetworkPacketHeader {
            source_ip_address,
            destination_ip_address: covopt_param!("M_216_36", 167772414),
            source_port: (covopt_param!("M_217_26", 1024)
                + (index % covopt_param!("M_217_42", 60000))) as u16,
            destination_port,
            protocol_number,
            packet_timestamp,
            packet_size,
    ...
    group.bench_function("packet_filter_throughput", |b| {
        ...
        b.iter(|| {
            ...
            for packet_header in packet_headers.iter() {
                ...
                decision_accumulator =
                    decision_accumulator.wrapping_add(decision.rule_identifier as u32);
            }
    ...
    group.bench_function("full_filter_and_compress_pipeline", |b| {
        b.iter(|| {
            ...
            for packet_header in packet_headers.iter() {
                let decision = firewall.evaluate_packet(black_box(packet_header));
                let written =
                    encoder.encode_log_entry(packet_header, decision, &mut local_log_buffer);
