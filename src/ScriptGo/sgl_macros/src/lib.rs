...
use std::str::FromStr;
use syn::{
    FnArg, ItemFn, ItemMod, Lit, LitInt, Meta, Pat, PatType, ReturnType, Token, Type,
...
pub fn sgl_compile(input: TokenStream) -> TokenStream {
    ...
    if let Lit::Str(str_lit) = input_lit {
        ...
        TokenStream::from(
            quote! { compile_error!("Expected a string literal containing SGL code"); },
        )
    }
}
...
fn build_cmd_match_arm(id: u32, func: &ItemFn) -> ProcessedCmd {
    ...
    let return_handler = if ret_type_str.contains("Result") && ret_type_str.contains("String") {
        ...
        && (ret_type_str.contains("Vec") || ret_type_str.contains("u8"))
    {
        quote! {
            match res {
                Ok(out_bytes) => {
        ...
        for item in items.iter_mut() {
            if let syn::Item::Fn(func) = item {
                ...
                func.attrs.retain(|attr| {
                    ...
                    if attr_str.contains("sgl_cmd")
                        || attr_str.contains("sgl_syscall")
                        || attr_str.contains("sgl_hardware_call")
                    {
                        is_cmd = true;
