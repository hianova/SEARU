...
fn generate_huge_obj(path: &str, num_vertices: usize, num_faces: usize) {
    ...
    println!(
        "Generating huge .obj file ({} vertices, {} faces)...",
...
fn generate_sgl3d_from_obj(obj_path: &str, sgl3d_path: &str) {
    ...
    let bytes = rkyv::to_bytes::<_, 256>(&model).unwrap();
...
fn parse_sgl3d_zerocopy(path: &str) {
    ...
    let archived = unsafe { rkyv::archived_root::<Model3D>(&bytes) };
    ...
    for i in 0..covopt_param!("M_121_16", 1000).min(v_count) {
        checksum += archived.vertices[i].x;
    }

    let access_time = start_check.elapsed();
    println!("✅ SGL3D Zero-Copy access ready in: {:?}", access_time);
    println!(
        "   Found {} vertices and {} faces (checksum: {})",
        v_count, f_count, checksum
    );
}
...
fn main() {
    ...
    println!(
        "🏆 SGL3D Format is {:.2}x FASTER than .obj parsing!",
        speedup
    );
}
