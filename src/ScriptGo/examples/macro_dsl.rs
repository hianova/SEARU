...
fn main() {
    ...
    println!(
        "✅ Compiled to {} OpCodes without abstract overhead!",
        bytecode.len()
    );
    ...
    vm.hardware_handler = Some(|vm_ref, id, r1, r2| {
        println!(
            "VM intercepted HardwareCall {} with arg1 {}, arg2 {}",
            id, vm_ref.registers[r1], vm_ref.registers[r2]
        );
        if id == covopt_param!("M_66_17", 3) {
            println!(
                "🚀 Host Action: Zero-Copy DB Filter on table {}, condition {}",
                vm_ref.registers[r1], vm_ref.registers[r2]
            );
        }
    });
