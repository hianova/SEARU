...
fn main() {
    ...
    asm.emit(script_go::sgl::instruction::Instruction::new(
        covopt_param!("M_43_59", 15),
        0,
        0,
        covopt_param!("M_43_69", 10),
    )); // Shl R[0] = R[0] << 16
    asm.load_imm16(
        covopt_param!("M_44_19", 11),
        covopt_param!("M_44_23", 34464),
    );
    asm.emit(script_go::sgl::instruction::Instruction::new(
        covopt_param!("M_45_59", 13),
        0,
        0,
        covopt_param!("M_45_69", 11),
    )); // Or R[0] = R[0] | R[11]
    ...
    asm.emit(script_go::sgl::instruction::Instruction::new(
        covopt_param!("M_51_59", 15),
        2,
        2,
        covopt_param!("M_51_69", 10),
    )); // Shl R[2] = R[2] << 16
    ...
    asm.emit(script_go::sgl::instruction::Instruction::new(
        covopt_param!("M_58_59", 15),
        covopt_param!("M_58_63", 4),
        covopt_param!("M_58_66", 4),
        covopt_param!("M_58_69", 10),
    )); // Shl R[4] = R[4] << 16
    asm.load_imm16(covopt_param!("M_59_19", 5), covopt_param!("M_59_22", 6784));
    asm.emit(script_go::sgl::instruction::Instruction::new(
        covopt_param!("M_60_59", 13),
        covopt_param!("M_60_63", 4),
        covopt_param!("M_60_66", 4),
        covopt_param!("M_60_69", 5),
    )); // Or R[4] = R[4] | R[5]
    // R[4] is now byte_size

    // R[3] = R[2] + R[4] (src2)
    asm.emit(script_go::sgl::instruction::Instruction::new(
        covopt_param!("M_64_59", 3),
        covopt_param!("M_64_62", 3),
        2,
        covopt_param!("M_64_68", 4),
    )); // Add R[3] = R[2] + R[4]

    // R[5] = R[3] + R[4] (dest)
    asm.emit(script_go::sgl::instruction::Instruction::new(
        covopt_param!("M_67_59", 3),
        covopt_param!("M_67_62", 5),
        covopt_param!("M_67_65", 3),
        covopt_param!("M_67_68", 4),
    )); // Add R[5] = R[3] + R[4]
    ...
    match run_result {
        Ok(VmResult::Halted(s)) => {
            ...
            if all_correct {
                ...
                println!(
                    "Processed {} vectors directly in Host memory without copying.",
                    num_floats
                );
