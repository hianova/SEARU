...
fn main() {
    ...
    for i in 0..covopt_macro::covopt_param!("M_50_16", 1000000) {
        db.push(Record {
            id: i,
            balance: (i * covopt_macro::covopt_param!("M_53_26", 17))
                % covopt_macro::covopt_param!("M_53_32", 5000),
            status: i % 2,
        });
    }
    ...
    vm.hardware_handler = Some(|vm: &mut ScriptVm, dest: usize, src: usize, op: usize| {
        ...
        if op == 0 {
            // get_balance
            vm.registers[dest] = (id * covopt_macro::covopt_param!("M_105_39", 17))
                % covopt_macro::covopt_param!("M_105_45", 5000);
        } else if op == 1 {
            // get_status
