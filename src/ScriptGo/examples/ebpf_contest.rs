...
fn main() {
    std::io::stdout()
        .write_fmt(format_args!(
            "🛡️  ScriptGo (SGL) Zero-Allocation eBPF Network Firewall Contest 🛡️"
        ))
        .unwrap();
    ...
    std::io::stdout()
        .write_fmt(format_args!(
            "Generating {} synthetic network packet headers...",
            packet_count
        ))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    ...
    std::io::stdout()
        .write_fmt(format_args!(
            "Executing Edge Network Firewall evaluation and zero-alloc binary flow logging..."
        ))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    ...
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!("✅ Firewall Execution Summary:"))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!(
    ...
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!("📊 Performance & Compression Metrics:"))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!(
    ...
    std::io::stdout()
        .write_fmt(format_args!(
            "  - Processing Speed:        {:.2} Million Packets/sec",
            throughput_packets_per_sec / 1_000_000.0
        ))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!(
            "  - Average Packet Latency:  {:.2} ns/packet",
    ...
    std::io::stdout()
        .write_fmt(format_args!(
            "  - Raw Metadata Size:       {:.2} MB ({} bytes)",
            raw_header_bytes as f64 / 1_048_576.0,
            raw_header_bytes
        ))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!(
            "  - Compressed Log Size:     {:.2} MB ({} bytes)",
