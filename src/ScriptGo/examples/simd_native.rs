...
fn main() {
    ...
    for i in 0..covopt_param!("M_43_16", 50) {
        ...
        vm.memory[i * covopt_param!("M_46_22", 4)
            ..i * covopt_param!("M_46_29", 4) + covopt_param!("M_46_33", 4)]
            .copy_from_slice(&val1);
        vm.memory[covopt_param!("M_47_18", 400) + i * covopt_param!("M_47_28", 4)
            ..covopt_param!("M_47_31", 400)
                + i * covopt_param!("M_47_41", 4)
                + covopt_param!("M_47_45", 4)]
            .copy_from_slice(&val2);
    }
    ...
    match vm.run(&code) {
        Ok(VmResult::Halted(steps)) => {
            ...
            let out_bytes: [u8; 4] = vm.memory
                [covopt_param!("M_64_47", 800)..covopt_param!("M_64_52", 804)]
                .try_into()
                .unwrap();
            println!(
                "First element of VecAdd result: {}",
                f32::from_le_bytes(out_bytes)
            );

            // Check dest memory (last element 49.0 + 2.0 = 51.0)
            let out_bytes_last: [u8; 4] = vm.memory[covopt_param!("M_68_52", 800)
                + covopt_param!("M_68_58", 49) * covopt_param!("M_68_63", 4)
                ..covopt_param!("M_68_66", 804)
                    + covopt_param!("M_68_72", 49) * covopt_param!("M_68_77", 4)]
                .try_into()
                .unwrap();
            println!(
                "Last element of VecAdd result: {}",
                f32::from_le_bytes(out_bytes_last)
            );
        }
        Err(e) => {
            println!("VM Error: {:?}", e);
