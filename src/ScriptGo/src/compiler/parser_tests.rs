...
mod tests {
    ...
    fn test_operator_precedence_comparison_and_logical() {
        ...
        if let Statement::LetDecl(_, _, expr) = &program.statements[0] {
            ...
            let cmp_lt =
                Expr::BinaryOp(Box::new(left_add), BinaryOperator::Lt, Box::new(right_mul));
            let cmp_eq = Expr::BinaryOp(
                Box::new(Expr::Identifier("x".into())),
                BinaryOperator::Eq,
                Box::new(Expr::Identifier("y".into())),
            );
            let expected = Expr::BinaryOp(Box::new(cmp_lt), BinaryOperator::And, Box::new(cmp_eq));
            assert_eq!(*expr, expected);
        } else {
            panic!("Expected LetDecl statement");
        }
    ...
    fn test_method_chaining_and_closing_paren_validation() {
        ...
        if let Statement::ExprStmt(expr) = &program.statements[0] {
            ...
            let expected =
                Expr::MethodCall(Box::new(inner_call), "c".into(), vec![Expr::IntLiteral(2)]);
            assert_eq!(*expr, expected);
        } else {
    ...
    fn test_ui_call_opcode_emission() {
        ...
        let ui_call_inst = bytecode
            .iter()
            .find(|inst| crate::opcode!(**inst) == OpCode::UiCall as u8);
        assert!(
            ui_call_inst.is_some(),
            "Expected UiCall opcode 36 in bytecode"
        );

        // Ensure opcode 254 is NOT emitted anywhere
        let invalid_inst = bytecode
            .iter()
            .find(|inst| crate::opcode!(**inst) == covopt_param!("M_236_81", 254));
