    ...
    fn parse_statement(&mut self) -> Result<Statement, String> {
        ...
        if self.match_token(&Token::Fn) {
            ...
            let has_arrow = if self.match_token(&Token::Arrow) {
                true
            } else if self.current_token() == Some(&Token::Minus)
                && self.peek_token() == Some(&Token::Gt)
            {
        ...
        if self.match_token(&Token::Equal) {
            ...
            if let Expr::Identifier(ident) = &expr {
                ...
                self.expect_token(&Token::Semicolon, "Expected ';' after assignment")?;
                let val =
                    Expr::BinaryOp(Box::new(expr.clone()), BinaryOperator::Add, Box::new(right));
            ...
            if let Expr::Identifier(ident) = &expr {
                ...
                self.expect_token(&Token::Semicolon, "Expected ';' after assignment")?;
                let val =
                    Expr::BinaryOp(Box::new(expr.clone()), BinaryOperator::Sub, Box::new(right));
                return Ok(Statement::Assign(ident.clone(), val));
            } else {
                return Err(self.error("Invalid assignment target"));
            }
        ...
        let mut expr = match self.current_token().cloned() {
            ...
            Some(Token::Identifier(v)) => {
                ...
                if self.match_token(&Token::LParen) {
                    ...
                    if is_macro {
                        return Err(
                            self.error("Expected '(' after macro name, found invalid fuzzy syntax")
                        );
                    }
                    Expr::Identifier(v)
                }
