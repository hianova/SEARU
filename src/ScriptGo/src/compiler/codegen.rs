...
use no_std_tool::structures::collections::HashMap;
use std::io::Write;

pub struct CodeGen {
    pub vars_reg: HashMap<String, u8>,
    ...
    pub fn compile(&mut self, prog: &Program) -> Result<Vec<VmInst>, String> {
                ...
                match &inst.op {
                    Op::LoadImm(val) => {
                        ...
                        if (0..covopt_param!("M_87_31", 256)).contains(&v) {
                            ...
                            let high = ((v >> covopt_param!("M_91_46", 8))
                                & covopt_param!("M_91_51", 255))
                                as u8;
                    ...
                    Op::Spawn(target_pc) => {
                        ...
                        let c = ((target_pc >> covopt_param!("M_305_47", 8))
                            & covopt_param!("M_305_52", 255)) as u8;
        ...
        for (idx, target_block, is_cond) in backpatch_jumps {
            if let Some(&target_pc) = block_starts.get(&target_block) {
                ...
                let high = ((target_pc >> covopt_param!("M_328_42", 8))
                    & covopt_param!("M_328_47", 255)) as u8;
                if is_cond {
        ...
        match expr {
            ...
            Expr::MacroCall(name, args) => {
                if name == "spawn" {
                    ...
                    let id = func.alloc_val();
                    self.append_inst(func, block_id, Instruction { id, op: Op::Yield });
                    return Ok(id);
                }
