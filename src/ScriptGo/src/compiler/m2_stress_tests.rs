...
mod tests {
    ...
    fn test_precedence_multiplication_over_addition() {
        ...
        assert_eq!(
            vm.registers[8], 1,
            "1 + 2 * 3 == 7 must evaluate to 1 (true)"
        );
    }
    ...
    fn test_precedence_left_associativity_subtraction() {
        ...
        assert_eq!(
            vm.registers[8], 1,
            "10 - 2 - 3 == 5 must evaluate to 1 (true)"
        );
    }
    ...
    fn test_syntax_error_unrecognized_chars() {
        ...
        for (input, ch) in invalid_inputs {
            ...
            assert!(
                res.is_err(),
                "Expected error for unrecognized char '{}'",
                ch
            );
    ...
    fn test_syntax_error_unterminated_strings() {
        ...
        for input in invalid_strings {
            ...
            assert!(
                res.is_err(),
                "Expected error for unterminated string in: {}",
                input
            );
            let err = res.unwrap_err();
            assert!(
    ...
    fn test_syntax_error_bad_floats() {
        let bad_floats = ["let f = 1.2.3;", "let f = 10.20.30.40;", "let f = 0.1.2;"];
    ...
    fn test_syntax_error_missing_semicolons() {
        let missing_semi_inputs = ["let x = 5 let y = 10;", "let x = 5", "x = 10", "return 42"];
        ...
        for input in missing_semi_inputs {
            ...
            if let Ok(tokens) = lexer.tokenize() {
                ...
                assert!(
                    res.is_err(),
                    "Expected parser error for missing semicolon in: {}",
                    input
                );
                let err = res.unwrap_err();
                assert!(
    ...
    fn test_method_call_tokenization_number_disambiguation() {
        ...
        assert_eq!(
            ...
            "123.foo() must tokenize as IntLiteral(123), Dot, Identifier('foo'), LParen, RParen"
        );
    }
    ...
    fn test_method_call_parsing_and_chaining() {
        ...
        assert_eq!(program.statements.len(), 1);
        if let Statement::ExprStmt(Expr::MethodCall(target, method_name, args)) =
    ...
    fn test_ui_call_opcode_emission_and_verification() {
        ...
        assert_eq!(
            OpCode::UiCall as u8,
            36,
            "OpCode::UiCall numeric value must be 36"
        );

        // Find instruction with opcode 36
        let ui_call_inst = bytecode
            .iter()
            .find(|inst| crate::opcode!(**inst) == covopt_param!("M_422_81", 36));
        assert!(
            ui_call_inst.is_some(),
            "Bytecode must contain OpCode 36 (UiCall)"
        );

        // Verify invalid/undefined opcode 254 is NOT in bytecode
        let invalid_inst = bytecode
            .iter()
            .find(|inst| crate::opcode!(**inst) == covopt_param!("M_426_81", 254));
        assert!(
            invalid_inst.is_none(),
            "Opcode 254 must not be present in compiled bytecode"
        );
    }
    ...
    fn test_ui_call_execution_in_vm() {
        ...
        assert!(
            UI_INVOKED.load(Ordering::SeqCst),
            "ui_handler must be invoked by OpCode 36"
        );
