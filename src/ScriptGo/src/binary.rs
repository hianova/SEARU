...
pub fn serialize_sgb(code: &[Instruction], max_fuel: u16, data_segment: &[u8]) -> Vec<u8> {
    ...
    let padding = (covopt_param!("M_32_19", 4)
        - (data_segment.len() % covopt_param!("M_32_45", 4)))
        % covopt_param!("M_32_51", 4);
...
pub fn deserialize_sgb(bytes: &[u8]) -> Result<(Vec<Instruction>, u16, Vec<u8>), &'static str> {
    ...
    let version = u16::from_le_bytes([
        bytes[covopt_param!("M_53_44", 4)],
        bytes[covopt_param!("M_53_54", 5)],
    ]);
    if version != 1 {
        return Err("Unsupported version");
    }

    let max_fuel = u16::from_le_bytes([
        bytes[covopt_param!("M_58_45", 6)],
        bytes[covopt_param!("M_58_55", 7)],
    ]);
    let data_len = u32::from_le_bytes([
        bytes[covopt_param!("M_59_45", 8)],
        bytes[covopt_param!("M_59_55", 9)],
        bytes[covopt_param!("M_59_65", 10)],
        bytes[covopt_param!("M_59_76", 11)],
    ]) as usize;
    let code_len = u32::from_le_bytes([
        bytes[covopt_param!("M_60_45", 12)],
        bytes[covopt_param!("M_60_56", 13)],
        bytes[covopt_param!("M_60_67", 14)],
        bytes[covopt_param!("M_60_78", 15)],
    ]) as usize;

    let padding = (covopt_param!("M_62_19", 4) - (data_len % covopt_param!("M_62_35", 4)))
        % covopt_param!("M_62_41", 4);
    let expected_len =
        covopt_param!("M_63_23", 16) + data_len + padding + code_len * covopt_param!("M_63_60", 4);
