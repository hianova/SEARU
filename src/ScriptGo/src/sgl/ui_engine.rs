...
impl UiDispatcher {
    ...
    pub fn dispatch(
        ...
        let val_a = if reg_a < covopt_param!("M_122_31", 256) {
            vm.registers[reg_a]
        } else {
            0
        };
        let val_b = if reg_b < covopt_param!("M_123_31", 256) {
            vm.registers[reg_b]
        } else {
            0
        };
        let val_c = if reg_c < covopt_param!("M_124_31", 256) {
            vm.registers[reg_c]
        } else {
            0
        };
        ...
        let payload_address = if val_c != 0 { val_c as usize } else { reg_c };
    ...
    fn handle_render_stream(
        ...
        if payload_address != 0 && payload_length > 0 {
            ...
            && let Ok(chunk_bytes) =
                vm.read_bytes(payload_address, covopt_param!("M_272_68", 1024), true)
