...
pub struct ScriptVm {
    ...
    _tracker: no_std_tool::diagnostics::debug::ScopedResource,
}
...
impl ScriptVm {
    ...
    pub fn get_ptr(&self, addr: usize, len: usize) -> Result<*const u8, VmError> {
        if addr < covopt_param!("M_76_18", 1024) {
            if addr
                .checked_add(len)
                .is_some_and(|end| end <= covopt_param!("M_77_62", 1024))
            {
                Ok(self.memory.as_ptr().wrapping_add(addr))
            } else {
                Err(VmError::MemoryAccessOutOfBounds {
            ...
            if offset
                .checked_add(len)
                .is_some_and(|end| end <= self.mmap_len)
            {
    ...
    pub fn get_mut_ptr(&mut self, addr: usize, len: usize) -> Result<*mut u8, VmError> {
        if addr < covopt_param!("M_96_18", 1024) {
            if addr
                .checked_add(len)
                .is_some_and(|end| end <= covopt_param!("M_97_62", 1024))
            {
    ...
    pub fn register_syscall_handler_ext(
        &mut self,
        handler: fn(&mut ScriptVm, usize, usize, usize),
    ) {
    ...
    pub fn read_bytes(
        ...
        len: usize,
        until_null: bool,
    ) -> Result<alloc::vec::Vec<u8>, VmError> {
        if until_null {
    ...
    pub fn write_string(
        ...
        let total_len = if null_terminate {
            bytes.len() + 1
        } else {
            bytes.len()
        };
    ...
    pub fn new() -> Self {
        Self {
            ...
            trace_head: 0,
            trace_count: 0,
            _tracker: no_std_tool::diagnostics::debug::ScopedResource::new(),
        }
    }
    ...
    pub fn new_boxed() -> Box<Self> {
        unsafe {
            ...
            core::ptr::addr_of_mut!((*ptr)._tracker)
                .write(no_std_tool::diagnostics::debug::ScopedResource::new());
            Box::from_raw(ptr)
        }
    }
        ...
        loop {
            ...
            match opcode {
                ...
                1 => {
                    ...
                    unsafe {
                        *self.registers.get_unchecked_mut(a) = crate::inst_b!(inst) as u32;
                    }
                }
                2 => {
                    let a = crate::inst_a!(inst);
                    unsafe {
                        *self.registers.get_unchecked_mut(a) = crate::inst_imm16!(inst) as u32;
                    }
                }
                3 => {
                    let a = crate::inst_a!(inst);
                ...
                29 => {
                    ...
                    if let Some(handler) = self.syscall_handler {
                        handler(self.registers[a], self.registers[b], self.registers[c]);
                    }
                }
                30 => {
                ...
                31 => {
                    ...
                    unsafe {
                        for i in 0..covopt_param!("M_569_36", 4) {
                            *ptr.add(i) = ((val >> (i * covopt_param!("M_570_56", 8)))
                                & covopt_param!("M_570_62", 255))
                                as u8;
                        }
                    }
                }
                32 => {
                    ...
                    if let Some(res) = no_std_tool::numerics::math::exp_approx_q16(val) {
                ...
                33 => {
                    ...
                    if let Some(res) = no_std_tool::numerics::math::rsqrt_approx_i32(val) {
                ...
                34 => {
                    let val = (self.registers[crate::inst_b!(inst)]
                        & covopt_param!("M_591_70", 255)) as i8;
                    if let Some(res) = no_std_tool::numerics::math::silu_approx_i8(val) {
                        self.registers[crate::inst_a!(inst)] =
                            (res as u32) & covopt_param!("M_593_78", 255);
                    } else {
                        return Err(VmError::MathError { pc: self.pc - 1 });
                    }
                }
                ...
                39 => {
                    ...
                    let byte_len = len.checked_mul(covopt_param!("M_655_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: dest,
                        },
                    )?;
                ...
                40 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_668_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: dest,
                        },
                ...
                41 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_681_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: src1,
                        },
                ...
                42 => {
                    ...
                    return Ok(VmResult::Spawn(
                        steps,
                        target_pc as u16,
                        crate::inst_a!(inst) as u8,
                    ));
                }
                43 => {
                    // Await
                    let resource_id = self.registers[crate::inst_b!(inst)];
                    return Ok(VmResult::Awaiting(
                        steps,
                        resource_id,
                        crate::inst_a!(inst) as u8,
                    ));
                ...
                45 => {
                    ...
                    self.registers[dst] =
                        no_std_tool::structures::compress::zigzag_encode_i32(delta);
                }
                46 => {
                    let dst = crate::inst_a!(inst);
                    let src = self.registers[crate::inst_b!(inst)];
                    let state = crate::inst_c!(inst);
                    let last = self.registers[state];
                    let delta = no_std_tool::structures::compress::zigzag_decode_u32(src);
    ...
    #[inline(always)]
    pub fn run_fast_with<N>(
        &mut self,
        code: &[Instruction],
        mut neural_handler: N,
        ...
        N: FnMut(&mut ScriptVm, usize, usize, usize),
    {
        ...
        loop {
            ...
            match opcode {
                ...
                31 => {
                    ...
                    unsafe {
                        for i in 0..covopt_param!("M_968_36", 4) {
                            *ptr.add(i) = ((val >> (i * covopt_param!("M_969_56", 8)))
                                & covopt_param!("M_969_62", 255))
                                as u8;
                        }
                    }
                }
                ...
                34 => {
                    let val = (self.registers[crate::inst_b!(inst)]
                        & covopt_param!("M_990_70", 255)) as i8;
                    if let Some(res) = no_std_tool::numerics::math::silu_approx_i8(val) {
                        self.registers[crate::inst_a!(inst)] =
                            (res as u32) & covopt_param!("M_992_78", 255);
                    } else {
                        return Err(VmError::MathError { pc: self.pc - 1 });
                    }
                }
                ...
                39 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_1052_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: dest,
                        },
                ...
                40 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_1065_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: dest,
                        },
                ...
                41 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_1078_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: src1,
                        },
    ...
    fn run_slow(&mut self, code: &[Instruction]) -> Result<VmResult, VmError> {
        ...
        while likely(self.pc < code.len()) {
            ...
            match crate::opcode!(inst) {
                ...
                31 => {
                    ...
                    unsafe {
                        for i in 0..covopt_param!("M_1373_36", 4) {
                            *ptr.add(i) = ((val >> (i * covopt_param!("M_1374_56", 8)))
                                & covopt_param!("M_1374_62", 255))
                                as u8;
                        }
                    }
                ...
                34 => {
                    let val = (self.registers[crate::inst_b!(inst)]
                        & covopt_param!("M_1399_70", 255)) as i8;
                    if let Some(res) = no_std_tool::numerics::math::silu_approx_i8(val) {
                ...
                39 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_1467_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: dest,
                        },
                ...
                40 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_1481_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: dest,
                        },
                ...
                41 => {
                    ...
                    let src1 = self.registers[crate::inst_b!(inst)] as usize;
                    let src2 = self.registers[crate::inst_c!(inst)] as usize;
                    let byte_len = len.checked_mul(covopt_param!("M_1495_51", 4)).ok_or(
                        VmError::MemoryAccessOutOfBounds {
                            pc: self.pc.wrapping_sub(1),
                            addr: src1,
                        },
                    ...
                    unsafe {
                        let mut sum = 0.0f32;
                        for i in 0..len {
                            let val1 = f32::from_le_bytes(core::ptr::read_unaligned(
                                src1_ptr.add(i * covopt_param!("M_1501_101", 4)) as *const [u8; 4],
                            ));
                            let val2 = f32::from_le_bytes(core::ptr::read_unaligned(
                                src2_ptr.add(i * covopt_param!("M_1502_101", 4)) as *const [u8; 4],
                            ));
                            sum += val1 * val2;
                        }
                        self.registers[dest_reg] = sum.to_bits();
...
mod tests {
    ...
    fn test_memory_load_store() {
        ...
        let code = [
            ...
            Instruction::new(
                OpCode::Load as u8,
                covopt_param!("M_1630_49", 4),
                2,
                covopt_param!("M_1630_55", 3),
            ),
            Instruction::new(OpCode::Halt as u8, 0, 0, 0),
        ];
        vm.run(&code).unwrap();
    ...
    fn test_math_approximations() {
        ...
        let code = [
            ...
            Instruction::new(
                OpCode::Silu as u8,
                covopt_param!("M_1649_49", 6),
                covopt_param!("M_1649_52", 3),
                0,
            ),
            Instruction::new(OpCode::Halt as u8, 0, 0, 0),
        ];
    ...
    fn test_audit() {
            ...
            let handle = std::thread::spawn(move || {
                ...
                let code = [
                    ...
                    Instruction::new(
                        ...
                        covopt_param!("M_1812_62", 4),
                        0,
                        covopt_param!("M_1812_68", 5),
                    ),
        ...
        for _ in 0..covopt_param!("M_1845_20", 4) {
            rx.recv_timeout(std::time::Duration::from_secs(covopt_param!(
                "M_1846_59",
                5
            )))
