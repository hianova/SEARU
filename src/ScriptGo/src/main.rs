...
use script_go::binary::{SGB_MAGIC, deserialize_sgb, serialize_sgb};
...
use script_go::sgl::instruction::Instruction;
use script_go::sgl::ui_engine::UiDispatcher;
use script_go::sgl::vm::{ScriptVm, TraceStep, VmResult};
use std::format;
use std::fs;
use std::io::Write;
...
impl CliConfig {
    pub fn parse_args(args: &[String]) -> Result<Self, String> {
        ...
        match subcommand_str {
            "run" => {
                if args.len() < covopt_param!("M_54_32", 3) {
            ...
            "check" => {
                if args.len() < covopt_param!("M_64_32", 3) {
            ...
            "compile" => {
                if args.len() < covopt_param!("M_74_32", 3) {
                ...
                let file = file
                    .ok_or_else(|| "Usage: sgo compile <file.sgo> [-o output.sgb]".to_string())?;
                Ok(CliConfig {
                    subcommand: SubCommand::Compile { file, output },
                })
            }
            "watch" => {
                if args.len() < covopt_param!("M_101_32", 3) {
            ...
            "--replay" => {
                if args.len() < covopt_param!("M_117_32", 3) {
            ...
            "--build" => {
                if args.len() < covopt_param!("M_127_32", 4) {
                    return Err("Usage: sgo --build <input> <output>".to_string());
                }
                Ok(CliConfig {
                    subcommand: SubCommand::Build {
                        input: args[2].clone(),
                        output: args[covopt_param!("M_133_37", 3)].clone(),
                    },
                })
            }
            ...
            _ => {
                    ...
                    || subcommand_str.ends_with(".sgb")
                {
                    ...
                    Err(format!(
                        "Unknown subcommand or file not found: {}",
                        subcommand_str
                    ))
                }
            }
...
pub fn run_cmd(file_path: &str) -> Result<VmResult, String> {
    ...
    let bytecode = if bytes.len() >= covopt_param!("M_202_37", 4)
        && bytes[0..covopt_param!("M_202_51", 4)] == SGB_MAGIC
    {
    ...
    writeln!(std::io::stdout(), "[RUN] Execution completed: {:?}", result).unwrap();
    Ok(result)
}
...
pub fn check_cmd(file_path: &str) -> Result<(), String> {
    ...
    if bytes.len() >= covopt_param!("M_229_22", 4)
        && bytes[0..covopt_param!("M_229_36", 4)] == SGB_MAGIC
    {
        deserialize_sgb(&bytes).map_err(|e| format!("SGB binary check failed: {}", e))?;
    } else {
        let source = String::from_utf8(bytes)
            .map_err(|e| format!("File content is not valid UTF-8: {}", e))?;
        compile_source(&source)?;
    ...
    writeln!(
        std::io::stdout(),
        "[CHECK] OK: {} (Syntax and types valid)",
        file_path
    )
    .unwrap();
    Ok(())
...
pub fn compile_cmd(file_path: &str, output_path: Option<String>) -> Result<PathBuf, String> {
    ...
    let binary_bytes = serialize_sgb(&bytecode, covopt_param!("M_248_48", 10000), &[]);
    ...
    fs::write(&target_path, &binary_bytes).map_err(|e| {
        format!(
            "Failed to write SGB file '{}': {}",
            target_path.display(),
            e
        )
    })?;
...
pub fn watch_cmd(file_path: &str, max_iterations: Option<usize>) -> Result<ScriptVm, String> {
    ...
    writeln!(
        std::io::stdout(),
        "[WATCH] Initial execution finished for {}: {:?}",
        file_path,
        result
    )
    .unwrap();
    ...
    loop {
        ...
        thread::sleep(Duration::from_millis(covopt_param!("M_314_44", 50)));

        let current_mtime = fs::metadata(file_path).and_then(|m| m.modified()).ok();
            ...
            && mtime > last_mtime
        {
            ...
            writeln!(
                std::io::stdout(),
                "[WATCH] File modification detected in {}. Recompiling...",
                file_path
            )
            .unwrap();
            ...
            match read_and_compile() {
                Ok(bytecode) => {
                    let bytecode: Vec<Instruction> = bytecode;
                    vm.hot_reload();
                    match vm.run(&bytecode) {
                        ...
                        Err(e) => {
                            writeln!(
                                std::io::stdout(),
                                "[WATCH] VM execution error post hot-reload: {:?}",
                                e
                            )
                            .unwrap();
                        }
                ...
                Err(e) => {
                    writeln!(std::io::stdout(), "[WATCH] Recompilation error: {}", e).unwrap();
                }
            }
        }
    }
...
pub fn replay_trace(path: &str) -> Result<(), String> {
    ...
    writeln!(std::io::stdout(), "⏱️  Replaying trace from: {}", path).unwrap();
    writeln!(
        std::io::stdout(),
        "--------------------------------------------------"
    )
    ...
    .unwrap();
    writeln!(std::io::stdout(), "✅ Trace replay completed successfully!").unwrap();
    Ok(())
}
...
pub fn build_asm(input_path: &str, output_path: &str) -> Result<(), String> {
    ...
    let code = parse_asm(&content).map_err(|e| format!("Assembly parse error: {:?}", e))?;
...
pub fn print_help() {
    writeln!(std::io::stdout(), "ScriptGo (SGL) DX CLI Tool").unwrap();
    writeln!(std::io::stdout(), "Usage:").unwrap();
    writeln!(
        std::io::stdout(),
        "  sgo run <file.sgo>                Compile and execute SGL script on ScriptVm"
    )
    .unwrap();
    writeln!(
        std::io::stdout(),
        "  sgo check <file.sgo>              Parse and type-check SGL script without running"
    )
    .unwrap();
    writeln!(
        std::io::stdout(),
        "  sgo compile <file.sgo> [-o out]   Compile SGL script into binary bytecode (.sgb)"
    )
    .unwrap();
    writeln!(
        std::io::stdout(),
        "  sgo watch <file.sgo> [--once]     Watch SGL script, recompile & hot-reload on change"
    )
    .unwrap();
    writeln!(
        std::io::stdout(),
        "  sgo --replay <trace.json>         Replay VM execution trace"
    )
    .unwrap();
    writeln!(
        std::io::stdout(),
        "  sgo --build <in.asm> <out.sgb>    Assemble SGL assembly source into bytecode"
    )
    .unwrap();
}
...
fn main() {
    ...
    if let Err(err) = run_cli(args) {
        std::writeln!(std::io::stdout(), "❌ ScriptGo CLI Error: {}", err).unwrap();
        std::process::exit(1);
    }
}
