...
use script_go::cli::{
    CliConfig, SubCommand, check_cmd, compile_cmd, compile_source, run_cmd, watch_cmd,
};
...
fn test_cli_check_sgo_script_validation() {
    ...
    assert!(invalid_check.is_err(), "Invalid script should fail check");
...
fn test_cli_watch_hot_reloading_state_preservation() {
    ...
    let vm =
        watch_cmd(watch_path.to_str().unwrap(), Some(1)).expect("Watch command initial run failed");

    assert_eq!(
        vm.registers[16], 99,
        "R[16] persistent register should be 99"
    );
    ...
    let result = reloaded_vm
        .run(&new_bytecode)
        .expect("Execution post reload failed");
    assert!(matches!(result, VmResult::Halted(_)));
...
fn test_cli_end_to_end_process_invocation() {
    ...
    let output = Command::new("cargo")
        .args(["run", "--quiet", "--", "run", sgo_file.to_str().unwrap()])
        .output()
