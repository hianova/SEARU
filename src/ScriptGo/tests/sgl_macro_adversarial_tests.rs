...
fn test_adversarial_combine_handlers_fallthrough_on_zero_return() {
    ...
    std::io::stdout()
        .write_fmt(format_args!("T1_A_COUNT: {}", count_a))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!("T1_B_COUNT: {}", count_b))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!("vm.registers[1]: {}", vm.registers[1]))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
...
fn test_adversarial_combine_handlers_fallthrough_on_invalid_string_param() {
    ...
    std::io::stdout()
        .write_fmt(format_args!("T2_A_COUNT: {}", count_a))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    std::io::stdout()
        .write_fmt(format_args!("T2_B_COUNT: {}", count_b))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    ...
    assert_eq!(
        count_a, 0,
        "Pkg A function body was not called due to param error"
    );
    assert_eq!(
        count_b, 1,
...
fn test_adversarial_cmd_id_resolution_fallback_to_register_index() {
    ...
    std::io::stdout()
        .write_fmt(format_args!("T3_A_COUNT: {}", count))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
