...
use no_std_tool::structures::compress::{
    TimeSeriesDecoder, TimeSeriesEncoder, TimestampDecoder, TimestampEncoder, leb128_decode_u32,
    leb128_encode_u32, zigzag_decode_u32, zigzag_encode_i32,
};
...
impl EdgeNetworkFirewall {
    ...
    pub fn evaluate_packet(&mut self, packet_header: &NetworkPacketHeader) -> PacketFilterDecision {
        ...
        if packet_header.destination_port == self.blocked_destination_port_telnet
            ...
            || (packet_header.source_ip_address & 0xFF00_0000) == 0x0A00_0000
        {
            return PacketFilterDecision {
...
impl ZeroAllocationFlowLogEncoder {
    pub fn new(initial_packet_header: &NetworkPacketHeader) -> Self {
        Self {
            ...
            source_ip_encoder: TimeSeriesEncoder::new(
                initial_packet_header.source_ip_address as i32,
            ),
        }
    }
    ...
    pub fn encode_log_entry(
        ...
        let timestamp_bytes = self
            ...
            .expect("encode timestamp failed");
        offset += timestamp_bytes;

        if output_buffer.len().saturating_sub(offset) < 10 {
            panic!("Insufficient remaining buffer capacity for encoding source ip");
        }
        let source_ip_bytes = self.source_ip_encoder.encode_next(
            packet_header.source_ip_address as i32,
            &mut output_buffer[offset..],
        ).expect("encode source_ip failed");
        offset += source_ip_bytes;

        if output_buffer.len().saturating_sub(offset) < 10 {
            panic!("Insufficient remaining buffer capacity for encoding packet size");
        }
        let packet_size_bytes = self.packet_size_encoder.encode_next(
            packet_header.packet_size as i32,
            &mut output_buffer[offset..],
        ).expect("encode packet_size failed");
        ...
        let destination_port_bytes = leb128_encode_u32(
            ...
            &mut output_buffer[offset..],
        ).expect("encode destination_port failed");
...
impl ZeroAllocationFlowLogDecoder {
    pub fn new(initial_packet_header: &NetworkPacketHeader) -> Self {
        Self {
            ...
            source_ip_decoder: TimeSeriesDecoder::new(
                initial_packet_header.source_ip_address as i32,
            ),
        }
    }
    ...
    pub fn decode_log_entry(
        ...
        let (packet_timestamp, timestamp_bytes) = self
            .timestamp_decoder
            .decode_next(&input_buffer[offset..])?;
        offset += timestamp_bytes;

        let (source_ip_i32, source_ip_bytes) = self
            .source_ip_decoder
            .decode_next(&input_buffer[offset..])?;
        offset += source_ip_bytes;

        let (packet_size_i32, packet_size_bytes) = self
            .packet_size_decoder
            .decode_next(&input_buffer[offset..])?;
        offset += packet_size_bytes;
        ...
        let (packed_action_protocol, action_bytes) = leb128_decode_u32(&input_buffer[offset..])?;
        offset += action_bytes;

        let (rule_identifier_zigzag, rule_bytes) = leb128_decode_u32(&input_buffer[offset..])?;
        offset += rule_bytes;
...
fn test_zero_allocations_empirical_verification() {
    ...
    std::io::stdout()
        .write_fmt(format_args!(
            "[EMPIRICAL VERIFICATION] Total Heap Allocations over {} packets: {} (bytes: {})",
            iterations, allocs, bytes
        ))
        .unwrap();
    std::io::stdout().write_all(b"\n").unwrap();
    assert_eq!(
        allocs, 0,
        "Zero-allocation claim failed! Detected {} heap allocations during packet filtering!",
        allocs
    );
}
...
fn test_boundary_ip_values_and_protocols() {
    ...
    assert_eq!(
        firewall.evaluate_packet(&pkt_zero_ip).rule_action,
    ...
    assert_eq!(
        firewall.evaluate_packet(&pkt_broadcast_ip).rule_action,
        FirewallRuleAction::Allow
    );
    ...
    assert_eq!(
        firewall.evaluate_packet(&pkt_subnet_start).rule_action,
    ...
    assert_eq!(
        firewall.evaluate_packet(&pkt_subnet_end).rule_action,
        FirewallRuleAction::Drop
    );
    ...
    assert_eq!(
        firewall.evaluate_packet(&pkt_outside_subnet).rule_action,
        FirewallRuleAction::Allow
    );

    // 5. Packet size boundaries: min valid = 20, max valid = 1500
    ts += 1000;
    let pkt_undersized = NetworkPacketHeader {
        packet_size: 19,
        packet_timestamp: ts,
        ..pkt_outside_subnet
    };
    assert_eq!(
        firewall.evaluate_packet(&pkt_undersized).rule_action,
        FirewallRuleAction::AnomalyAlert
    );

    ts += 1000;
    let pkt_min_size = NetworkPacketHeader {
        packet_size: 20,
        packet_timestamp: ts,
        ..pkt_outside_subnet
    };
    assert_eq!(
        firewall.evaluate_packet(&pkt_min_size).rule_action,
        FirewallRuleAction::Allow
    );

    ts += 1000;
    let pkt_max_size = NetworkPacketHeader {
        packet_size: 1500,
        packet_timestamp: ts,
        ..pkt_outside_subnet
    };
    assert_eq!(
        firewall.evaluate_packet(&pkt_max_size).rule_action,
        FirewallRuleAction::Allow
    );

    ts += 1000;
    let pkt_oversized = NetworkPacketHeader {
        packet_size: 1501,
        packet_timestamp: ts,
        ..pkt_outside_subnet
    };
    assert_eq!(
        firewall.evaluate_packet(&pkt_oversized).rule_action,
        FirewallRuleAction::AnomalyAlert
    );

    // 6. Protocol 255 (Reserved)
    ts += 1000;
    let pkt_proto_255 = NetworkPacketHeader {
        protocol_number: 255,
        packet_timestamp: ts,
        ..pkt_outside_subnet
    };
    assert_eq!(
        firewall.evaluate_packet(&pkt_proto_255).rule_action,
        FirewallRuleAction::AnomalyAlert
    );

    // 7. Blocked ports: Telnet (23) and SMB (445)
    ts += 1000;
    let pkt_telnet = NetworkPacketHeader {
        destination_port: 23,
        packet_timestamp: ts,
        ..pkt_outside_subnet
    };
    assert_eq!(
        firewall.evaluate_packet(&pkt_telnet).rule_action,
        FirewallRuleAction::Drop
    );

    ts += 1000;
    let pkt_smb = NetworkPacketHeader {
        destination_port: 445,
        packet_timestamp: ts,
        ..pkt_outside_subnet
    };
    assert_eq!(
        firewall.evaluate_packet(&pkt_smb).rule_action,
        FirewallRuleAction::Drop
    );
}
...
fn test_small_output_buffer_panics() {
    ...
    assert!(
        res.is_err(),
        "Expected panic on buffer overflow when output buffer is too small for LEB128!"
    );
...
fn test_empty_stream_and_high_throughput_integrity() {
    ...
    for i in 1..=50000 {
        ...
        let (decoded_header, decoded_decision, read_bytes) = decoder
            .decode_log_entry(&log_buf[..bytes_written])
