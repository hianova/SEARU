...
use script_go::{SglIoRegisterExt, SglNetRegisterExt, sgl_combine_handlers, sgl_package};
use std::io::Write;
...
fn test_ac1_macro_mock_package_syscall_and_hardware_call() {
    ...
    vm.write_string(covopt_param!("M_61_20", 100), "valid", true)
        .unwrap();
    ...
    let hw_inst = Instruction::new(
        OpCode::HardwareCall as u8,
        1,
        2,
        covopt_param!("M_66_69", 3),
    );
    ...
    let sys_inst = Instruction::new(
        OpCode::SysCall as u8,
        covopt_param!("M_82_59", 4),
        covopt_param!("M_82_62", 5),
        covopt_param!("M_82_65", 6),
    );
...
fn test_ac2_workspace_members_sgl_net_and_sgl_io() {
    ...
    vm.write_string(
        covopt_param!("M_104_20", 100),
        "https://boundary.test/api",
        true,
    )
    .unwrap();
    vm.registers[1] = 0;
    ...
    let inst_net = Instruction::new(
        OpCode::HardwareCall as u8,
        1,
        2,
        covopt_param!("M_109_70", 3),
    );
...
fn test_boundary_invalid_memory_addresses_no_panic() {
    for &invalid_addr in &INVALID_ADDRESSES {
        // Fast mode
        {
            ...
            assert_eq!(
                vm.registers[1], 0,
                "Failed for invalid address {:#X} in fast mode",
                invalid_addr
            );
        }
        ...
        // Tracing / Slow mode (tests run_slow synchronization)
        {
            ...
            assert_eq!(
                vm.registers[1], 0,
                "Failed for invalid address {:#X} in slow mode",
                invalid_addr
            );
        }
    }
...
fn test_boundary_vm_load_store_invalid_addresses_no_panic() {
    for &invalid_addr in &INVALID_ADDRESSES {
        ...
        // Load from invalid address
        {
            ...
            let load_inst =
                Instruction::new(OpCode::Load as u8, 1, 2, covopt_param!("M_180_71", 3));
        ...
        // Store to invalid address
        {
            ...
            let store_inst =
                Instruction::new(OpCode::Store as u8, 1, 2, covopt_param!("M_198_73", 3));
...
fn test_boundary_malformed_utf8_no_panic() {
    ...
    let invalid_utf8 = [
        covopt_param!("M_218_24", 255),
        covopt_param!("M_218_29", 254),
        covopt_param!("M_218_34", 253),
        covopt_param!("M_218_39", 128),
        0x00,
    ]; // Null-terminated invalid UTF-8 sequence
    vm.write_bytes(covopt_param!("M_219_19", 100), &invalid_utf8)
        .unwrap();
...
fn test_boundary_out_of_bounds_register_indices_no_panic() {
    ...
    vm.write_string(covopt_param!("M_261_20", 100), "valid", true)
        .unwrap();
    vm.registers[1] = 0;
    vm.registers[2] = 1;
...
fn test_boundary_host_function_panic_recovery() {
    ...
    vm.write_string(covopt_param!("M_286_20", 100), "panic_trigger", true)
        .unwrap();
...
fn test_boundary_combined_handlers_no_panic() {
    let mut vm = ScriptVm::new();
    let combined = sgl_combine_handlers!(mock_boundary_hw::dispatch, mock_boundary_sys::dispatch);
