...
fn test_ui_engine_sgo_script_execution() {
    let source_code =
        std::fs::read_to_string("tests/ui_sample.sgo").expect("Failed to read tests/ui_sample.sgo");
    ...
    let bytecode = codegen
        .compile(&program)
        .expect("CodeGen compilation failed");
    ...
    let ui_dispatcher = vm
        .get_ui_dispatcher()
        .expect("UiDispatcher missing from VM");

    assert_eq!(ui_dispatcher.mount_count, 1, "Mount count should be 1");
    assert_eq!(
        ui_dispatcher.event_signal_count, 1,
        "Event signal count should be 1"
    );
    assert_eq!(
        ui_dispatcher.prop_update_count, 1,
        "Prop update count should be 1"
    );
    assert_eq!(ui_dispatcher.render_count, 1, "Render count should be 1");

    assert!(
        ui_dispatcher.components.contains_key(&1),
        "Component 1 should be mounted"
    );
    assert_eq!(
        ui_dispatcher.event_queue.len(),
        1,
        "One IPC event signal should be queued"
    );
    assert!(
        ui_dispatcher.last_rendered_output.is_some(),
        "Rendered output should be produced"
    );
}
...
fn test_ui_engine_cmd_lifecycle_and_state_sync() {
    ...
    let component = ui_dispatcher
        .components
        .get(&covopt_param!("M_95_50", 42))
        .expect("Component 42 missing");
    assert_eq!(component.id, 42);
    assert_eq!(
        component.props.get("label").map(|s| s.as_str()),
        Some("Submit")
    );
    assert_eq!(
        component.props.get("color").map(|s| s.as_str()),
        Some("blue")
    );
    assert_eq!(
        component.props.get("enabled").map(|s| s.as_str()),
        Some("true")
    );
...
fn test_ui_engine_large_document_streaming_100mb() {
    ...
    let chunk_size = covopt_param!("M_117_21", 10)
        * covopt_param!("M_117_26", 1024)
        * covopt_param!("M_117_33", 1024); // 10MB chunk size
    ...
    for chunk_index in 0..total_chunks {
        dispatcher
            .stream_buffer
            .append_chunk(&sample_markdown_chunk);
    ...
    let vm = ScriptVm::new();
    dispatcher
        .dispatch(&vm, 1, covopt_param!("M_141_32", 4), 0)
        .expect("Render dispatch failed");
