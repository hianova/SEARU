...
impl RhythmEngine {
    ...
    pub fn generate_rhythm(mut track_score: Vec<BarScore>, profile: &crate::profile::ArtistProfile) -> Vec<BarScore> {
        ...
        for bar in 0..track_score.len() {
            ...
            for step in 0..16 {
                ...
                let kick_downbeat = if profile.culture.rhythmic_grid == "4/4" { 4 } else { 3 }; // 4/4 vs Polyrhythm (3/4 overlay)
                ...
                if kick_vel > 0.0 {
                    track_score[bar].kick[step] = StepEvent { velocity: kick_vel, length: 1.0, pitch: 0.0 };
                }
                ...
                if bass_vel > 0.0 {
                    track_score[bar].bass[step] = StepEvent { velocity: bass_vel, length: bass_len, pitch: 0.0 };
                }
                ...
                if step == 0 {
                    ...
                    if pad_vel > 0.0 {
                        track_score[bar].pad[step] = StepEvent { velocity: pad_vel, length: 16.0, pitch: 0.0 };
                    }
                }
                ...
                if rng.random::<f32>() < arp_prob {
                    ...
                    track_score[bar].arp[step] = StepEvent { velocity: arp_vel, length: 1.0, pitch: 0.0 };
                }
                ...
                if hat_vel > 0.0 {
                    ...
                    track_score[bar].hat[step] = StepEvent { velocity: hat_vel.clamp(0.0, 1.0), length: 1.0, pitch: 0.0 };
                }
            }
        }
