...
impl MelodyEngine {
    ...
    pub fn generate_melody(mut track_score: Vec<BarScore>, profile: &crate::profile::ArtistProfile) -> Vec<BarScore> {
        ...
        for bar in 0..track_score.len() {
            ...
            println!("🧬 Melody Engine: Evolving Fractal Melody for Bar {}...", bar);
            ...
            let (_, best_genes) = TheCrucible::anneal(
                ...
                |current_genes| {
                    ...
                    if current_genes[14].current_value >= 0.0 { penalty += 15.0; }
                    if current_genes[15].current_value >= 0.0 { penalty += 30.0; }
            ...
            for i in 0..16 {
                ...
                if val >= 0.0 {
                    ...
                    let vel = Orchestrator::smooth_step(0.2, 0.8, energy).max(0.3) * (0.8 + rng.random::<f32>() * 0.2);
                    
                    // Calculate note length (legato until next note)
                    let mut length = 1.0;
                    for j in (i + 1)..16 {
                        if best_genes[j].current_value >= 0.0 { break; }
                        length += 1.0;
                    }
