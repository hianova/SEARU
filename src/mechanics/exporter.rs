...
impl TrussExporter {
    pub fn to_svg_string(truss: &Truss) -> String {
        ...
        for bar in &truss.bars {
            ...
            let color = if bar.stress > 80.0 { "#e74c3c" } else { "#3498db" };
        ...
        for node in &truss.nodes {
            let color = if node.fixed { "#2c3e50" } else if node.force_y > 0.0 { "#e67e22" } else { "#95a5a6" };
            let radius = if node.fixed { 8.0 } else { 5.0 };
            
            out.push_str(&format!(
                "  <circle cx=\"{:.1}\" cy=\"{:.1}\" r=\"{:.1}\" fill=\"{}\" />\n",
                node.x * scale + dx, node.y * scale + dy, radius, color
            ));
        }
